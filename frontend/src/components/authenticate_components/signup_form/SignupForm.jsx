...
export default function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    await signup(name, email, password, confirmPassword, 'end-user', mobileNo)
    e.preventDefault()
  }

  const signup = async (name, email, password, confirmPassword, role, mobileNo) => {
    //alert(mobileNo)
    setIsLoading(true)
    setError(null)
    ...
    const response = await fetch('/api/user/signup', {
      ...
      body: JSON.stringify({ name, email, password, confirmPassword, role, mobileNo })
    })
